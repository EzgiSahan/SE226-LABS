...
def passGen(number):
    ...
    x = "".join(List)
    print("Initial password: " + x)
    return x


print("Enter number between 3 to 7")
user_number = int(input())
passGen(user_number)


def rep_with_upper(stringValue):
    randomValue = random.choice(stringValue)
    print(''.join(str.lower(randomValue)) + "-->" + ''.join(str.upper(randomValue)))
    return "".join(str.upper(randomValue))


rep_with_upper("".join(List))


def swap_letters(stringValue):
    start = stringValue[0]
    start1 = stringValue[1]
    end = stringValue[-1]
    end1 = stringValue[-2]
    swapString = end1 + end + stringValue[1:-1] + start + start1
    return swapString


print("Final version of the password: " + swap_letters("".join(List)))


def search_letter(stringValue, stringValue1):
    if stringValue == "".join(List):
        if str(stringValue).__contains__(str(stringValue1)):
            return True
        else:
            return False
...
print("Enter key: ")
key = input()
print(search_letter("".join(List), key))